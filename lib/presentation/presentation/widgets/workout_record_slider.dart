...
Widget workoutRecordSlider() {
  ...
  final WorkoutRecordController workoutRecordController =
      Get.put(WorkoutRecordController());
...
List<Widget> workOutRecordSliderCell(
    WorkoutRecordController workoutRecordController) {
  var nowDay = DateTime.now().weekday;
  List<Widget> list = [];
  for (int i = 0; i < 7; i++) {
    if (workoutRecordController.jsonWorkWeekTotalObject != null)
      for (int j = 0;
          j < workoutRecordController.jsonWorkWeekTotalObject.length;
          j++) {
