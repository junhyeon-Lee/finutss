...
class _MapPageState extends State<MapPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    centerButtonAnimation = CurvedAnimation(parent: _animationController, curve: Curves.fastLinearToSlowEaseIn);
          ...
          body: SafeArea(
            child: GetBuilder<MapController>(builder: (mapController) {
              return GetBuilder<SensorController>(builder: (sensorController) {
                ...
                if ((authController.user?.workoutType ?? "Riding") == "Riding") {
                  speed = sensorController.mWheel.length > 0
                      ...
                      : (sensorController.mCrank['crankCadence'] ?? 0) *
                          sensorController.sensitivityList[sensorController.sensitivity];
                } else {
                ...
                if (sensorController.mWheel.length > 0 || sensorController.mCrank.length > 0) {}
                            ...
                            onPageFinished: (url) {
                              ...
                              Timer(
                                  ...
                                  () => _controller!.webViewController
                                      .evaluateJavascript('setPath("$trackId", "$type")'));
                              ...
                              Timer(
                                  ...
                                  () => GlobalBloc().setSubscription(WorkOutQueries.NEW_WORKOUT, {'track': "track"},
                                          callback: (result) {
                                        result.data!['newWorkOut'].removeWhere(
                                            (item) => item['writer']['id'] == authController.user?.userId);
                                        var sendMsg = jsonEncode(result.data!['newWorkOut']);
                                        _controller!.webViewController.evaluateJavascript('setUser($sendMsg)');
                                      }));
                            },
                            javascriptChannels: <JavascriptChannel>[
                              JavascriptChannel(
                                  ...
                                  onMessageReceived: (JavascriptMessage msg) async {
                                    ...
                                    sensorController.setPhaseToWorkout(data['phase']);
                                    GlobalBloc().queryMutate(WorkOutQueries.UPDATE_WORKOUT, {
                                      'data': {
                                        ...
                                        'kcal': data['kcal'],
                                        'workoutType': authController.user?.workoutType ?? "Riding"
                                      },
                                    });
                                    isSend = true;
                                  }),
                              JavascriptChannel(
                                  ...
                                  onMessageReceived: (JavascriptMessage msg) async {
                                    ...
                                    await GlobalBloc().queryMutate(WorkOutQueries.UPDATE_WORKOUT, {
                                      'data': {
                                        'id': sensorController.workout['id'],
                                        'phase': data['phase'],
                                        'duration': data['duration'],
                            ...
                            children: [
                              Container(
                                ...
                                child: appbar(
                                    ...
                                    isGoingBack: false,
                                    titleWidget: InkWell(
                                      child: timerWidget(duration: mapController.duration),
                                      onTap: () {
                                        _onChangeSpeed(_controller!, context, 500);
                                      },
                                    )),
                              ),
                              size10,
                              Padding(
                                ...
                                child: allUserRunDistance(mapController.allUserDistance, speed.toInt()),
                              ),
                              Padding(
                                padding: EdgeInsets.only(top: 10.0.h, left: 20.w),
                              ...
                              children: [
                                ...
                                if (isClicked)
                                  Column(
                                    children: [
                                      Transform.translate(
                                        offset: Offset(toRightAnimation.value, 0),
                                        child: FadeTransition(
                                          opacity: _translationController.drive(CurveTween(curve: Curves.easeOut)),
                                          child: Padding(
                                      ...
                                      Transform.translate(
                                        offset: Offset(0, toTopAnimation.value),
                                        child: FadeTransition(
                                          opacity: _translationController.drive(CurveTween(curve: Curves.easeOut)),
                                          child: Column(
                                            children: [
                                              Row(
                                                ...
                                                children: [
                                                  circleIcons(
                                                      ...
                                                      iconColor: Color(0xff3BCCE1),
                                                      onPressedFunction: () {
                                                        Get.dialog(ConnectionDialogCombined(),
                        ...
                        child: Padding(
                          ...
                          child: runDistance(
                              ...
                              ratio: sensorController.workout == null
                                  ? 0.0
                                  : sensorController.workout["phase"].toDouble()),
                        ),
                      ),
                    ],
                  ),
  ...
  void _setMyMarker(WebViewPlusController controller) async {
    ...
    var data = '{"image": "${authController.user?.image?.url ?? "https://finutss.s3.ap-northeast-2.amazonaws.com/assets/finutss_default_profile.png"}", "type": "${authController.user?.workoutType ?? "Riding"}"}';
  ...
  Future<void> createWorkOut(trackId) async {
    if (authController.user == null) return;
    print('-= ADD_WORKOUT');
    var workout = await GlobalBloc().queryMutate(WorkOutQueries.ADD_WORKOUT, {
  ...
  buildMassage() {
    return GetBuilder<ChatController>(builder: (controller) {
      ...
      return Column(
        children: [for (var item in printMsg) messageWidget(message: item["description"], userInfo: item["writer"])],
      );
    });
  }
}
