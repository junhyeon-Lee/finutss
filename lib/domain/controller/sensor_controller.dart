...
class SensorController extends GetxController {
  ...
  initValue() async {
    circumference = await LocalDB.getInt("circumference") ?? 2268;

    ///TODO
    sensitivity = await LocalDB.getInt("sensitivity") ?? 0.27.toInt();
  }
  ...
  Future<void> startScan() async {
    ...
    try {
      ...
      //     timeout: Duration(seconds: 6), withServices: [rsc_uuid, pad_uuid]);


      if ((authController.user?.workoutType ?? '').toLowerCase() == Constants.riding.toLowerCase()) {
        FlutterBlue.instance.startScan(timeout: Duration(seconds: 6), withServices: [csc_uuid]);
      } else if ((authController.user?.workoutType ?? '').toLowerCase() == Constants.running.toLowerCase()) {
        FlutterBlue.instance.startScan(timeout: Duration(seconds: 6), withServices: [rsc_uuid, pad_uuid]);
      } else {
        FlutterBlue.instance.startScan(timeout: Duration(seconds: 6), withServices: [csc_uuid, rsc_uuid, pad_uuid]);
      }
    } catch (e) {
  ...
  Future<void> initDevice(BluetoothDevice device) async {
    try {
      userHeight = authController.user?.usersDetails?.height.toInt() ?? 160;
      ...
      if (typeCsc.isNotEmpty) {
        ...
        if (service.uuid == csc_uuid) {
          ...
          var bc = characteristics.singleWhere(
              (bc) => bc.uuid == csc_measurement_characteristic_uuid);
      ...
      print('-------init device exception----${e.toString()}');
      snackBar('initDevice :$e');
      print(e);
    }
  }
  ...
  Future<void> setSpeedNotification(void Function(double p1) callback) async {
    ...
    var characteristic = service.characteristics
        .singleWhere((bc) => bc.uuid == csc_measurement_characteristic_uuid);
    characteristic.setNotifyValue(true);
    deviceSpeedStreamSubscription = characteristic.value.listen((value) {
      if (value.length > 0) {
        var wheelRevolutions = value.length > 0
            ? (value[1] + (256 * value[2]) + (4096 * value[3]))
            : 0;
        var lastWheelEventTime =
            value.length > 0 ? value[5] + (256 * value[6]) : 0;
        onWheelMeasurementReceived(
            wheelRevolutions, lastWheelEventTime, callback);
      }
    });
  }
  ...
  Future<void> setCadenceNotification(void Function(double p1) callback) async {
    ...
    deviceCadenceStreamSubscription = characteristic.value.listen((value) {
      if (value.length > 0) {
        var crankRevolutions =
            value.length > 0 ? (value[1] + (256 * value[2])) : 0;
        var lastCrankEventTime =
            value.length > 0 ? value[3] + (256 * value[4]) : 0;
        onCrankMeasurementReceived(
            crankRevolutions, lastCrankEventTime, callback);
      }
    });
  }
  ...
  Future<void> setRSCNotification(void Function(double p1) callback) async {
    ...
    var characteristic = service.characteristics
        .singleWhere((bc) => bc.uuid == rsc_measurement_characteristic_uuid);
    characteristic.setNotifyValue(true);
  ...
  Future<void> setPadNotification(void Function(double p1) callback) async {
    ...
    var characteristic = service.characteristics
        .singleWhere((bc) => bc.uuid == pad_measurement_characteristic_uuid);
    characteristic.setNotifyValue(true);
        ...
        if (value[8] == 0) {
          ...
          if (now - mLastPadEventTime > 200) {
            ...
            if (now - mLastPadEventTime > 1000) {
              onPadMeasurementReceived(padRevolutions,
                  DateTime.now().millisecondsSinceEpoch, callback);
            }
          }
        }
  ...
  void onCrankMeasurementReceived(int crankRevolutions, int lastCrankEventTime,
      void Function(double p1) callback) {
    ...
    if (mLastCrankRevolutions >= 0) {
      ...
      if (lastCrankEventTime < mLastCrankEventTime)
        timeDifference =
            (65535 + lastCrankEventTime - mLastCrankEventTime) / 1024.0; // [s]
      else
        timeDifference =
            (lastCrankEventTime - mLastCrankEventTime) / 1024.0; // [s]

      crankCadence = (crankRevolutions - mLastCrankRevolutions) *
          60.0 /
          timeDifference; // [revolutions/minute]
      if (crankCadence > 0) {
        final double gearRatio =
            mWheelCadence >= 0 ? mWheelCadence / crankCadence : 0.0;
      } else {
    ...
    if (wheelDevice == null)
      callback(mCrank['crankCadence'] * sensitivityList[sensitivity]);
    // if (wheelDevice == null) callback(mCrank['crankCadence'] * 1.0);
    // update();
  }

  void onWheelMeasurementReceived(int wheelRevolutions, int lastWheelEventTime,
      void Function(double p1) callback) {
    ...
    if (mLastWheelRevolutions >= 0) {
      ...
      if (lastWheelEventTime < mLastWheelEventTime)
        timeDifference =
            (65535 + lastWheelEventTime - mLastWheelEventTime) / 1024.0; // [s]
      else
        timeDifference =
            (lastWheelEventTime - mLastWheelEventTime) / 1024.0; // [s]

      final num distanceDifference =
          (wheelRevolutions - mLastWheelRevolutions) *
              circumference /
              1000.0; // [m]
      final num totalDistance =
          wheelRevolutions * circumference / 1000.0; // [m]
      final num distance = (wheelRevolutions - mInitialWheelRevolutions) *
          circumference /
          1000.0; // [m]
      final num speed = distanceDifference / timeDifference; // [m/s]
      mWheelCadence = (wheelRevolutions - mLastWheelRevolutions) *
          60.0 /
          timeDifference; // [revolutions/minute]
  ...
  void onPadMeasurementReceived(int padRevolutions, int lastPadkEventTime,
      void Function(double p1) callback) {
    // print('onCrankMeasurementReceived');
    double speed = 0;
    ...
    speed =
        (((padRevolutions - mLastPadRevolutions) * (userHeight * 0.2 / 100)) *
                3.6) /
            timeDifference;
...
/* -------------------------------- functions ------------------------------- */
  selectWheelSize(int wheelSize) {
    circumference = wheelSize;
    LocalDB.setInt("circumference", wheelSize);
    update();
  }

  increaseSpeedCalibration() {
    if ((sensitivity + 1) > 9) return;
    sensitivity = sensitivity + 1;
    LocalDB.setInt("sensitivity", sensitivity);
    update();
  }
  ...
  callApiNewWorkOut(Map<String, dynamic> params) {
    GlobalBloc()
        .queryMutate(WorkOutQueries.NEW_WORKOUT, params)
        .then((response) {
  ...
  void setWorkoutType(String type) {
    authController.user?.workoutType = type;
    if (type == 'Riding') {
      devicePad?.disconnect();
      deviceRSC?.disconnect();
    } else {
